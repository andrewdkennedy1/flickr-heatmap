...
export default function Home() {
  ...
  const stats = useMemo(() => {
    ...
    return { totalUploads, activeDays, peakCount, peakDate };
  }, [data]);
  ...
  const handleFetch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (result.data.length === 0) {
        ...
        const sortedData: ActivityData[] = result.data.sort(
          (a: ActivityData, b: ActivityData) => new Date(a.date).getTime() - new Date(b.date).getTime(),
        );
        setData(sortedData);
      }
    } catch (err: unknown) {
      ...
      <div className="relative z-10 mx-auto flex max-w-6xl flex-col gap-12 px-6 py-10 md:py-14">
          ...
          <div className="space-y-6">
            ...
            >
              ...
              <p className="max-w-2xl text-lg text-slate-300">
                Paste any Flickr username and watch the last year of uploads transform into a contribution-style
                calendar. Log in to include private uploads in the visualization.
              </p>
            </motion.div>

            <div className="grid gap-4 sm:grid-cols-2">
              {[{
                title: 'Year-long coverage',
                icon: <Calendar className="text-emerald-300" size={18} />,
                description: 'We look back 365 days to capture every upload streak and lull.',
              }, {
          ...
          >
            ...
            <form onSubmit={handleFetch} className="relative space-y-6">
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                ...
                >
                  {loading ? <Loader2 className="inline-block animate-spin" size={20} /> : 'Generate heatmap'}
                </button>
                <button
                  type="button"
                  onClick={loadDemo}
        ...
        <section className="grid gap-4 rounded-3xl border border-slate-800/60 bg-slate-950/50 p-6 md:grid-cols-2">
          {[{
            ...
            body: 'Every photo upload is counted on the day it was posted. We track the last 12 months for a GitHub-style view.',
            ...
            body: 'Unsure where to start? nasa-goddard, nationalparkservice, and leica_camera make great demos.',
            ...
            >
              <div className="grid gap-4 md:grid-cols-3">
                <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-5">
                  ...
                  <p className="mt-2 text-xs text-slate-500">From the last 365 days of activity.</p>
                </div>
                <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-5">
                  <div className="mb-2 flex items-center gap-2 text-slate-400">
                    <Info size={18} />
